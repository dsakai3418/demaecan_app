...
def process_csv_data(input_df):
    try:
        # 日付カラムをdatetime型に変換
        # エラーが発生する可能性のあるカラムのみを変換し、errors='coerce'で不正な値をNaTに変換
...
if uploaded_file is not None:
    ...
    try:
        # アップロードされたファイルを一度bytesとして読み込み、io.StringIOでラップしてread_csvに渡す
        # encoding='utf-8'を優先し、失敗したら'shift_jis'を試す
            ...
            if st.button("データ処理を開始"):
                with st.spinner("データ処理中..."):
                    ...
                    if processed_df is not None:
                        ...
                        # Windows向け (Shift-JIS)
                        csv_windows = processed_df.to_csv(index=False, encoding='shift_jis')
                        st.download_button(
                        ...
                        csv_mac = processed_df.to_csv(index=False, encoding='utf-8-sig')
                        st.download_button(
